...
/**
 * BurariC21harikiGSP command.
 * 葛飾区情報局のコラム設定値を取得
 * 指定されたGoogleスプレッドシートからデータを取得し、JSONファイルとして保存します。
 */
class BurariC21harikiGSPCommand extends Command
{
    /**
     * @var string スプレッドシートID
     */
    private const SPREADSHEET_ID = '1pcjfLaudlRUcwrPaeeZnWi_e0xGYlsPEivUVMo3yDrI';

    /**
     * @var string 出力先ファイルパス
     */
    private const OUTPUT_FILE = ROOT . DS . 'temp' . DS . 'cache' . DS . 'nara_article.json';
    private const OUTPUT_FILE_PRO = "/var/www/workspace/burari/DocumentRoot/wp-content/plugins/follow-up/data/nara_estate.json";
    ...
    public function buildOptionParser(ConsoleOptionParser $parser): ConsoleOptionParser
    {
        ...
        $parser->setDescription('Googleスプレッドシートから葛飾区情報局の記事データを取得し、JSONファイルに保存します。');
